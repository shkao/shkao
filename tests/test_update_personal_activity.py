...
class PersonalActivityTests(unittest.TestCase):
    ...
    def test_graph_uses_monet_teal_in_both_themes(self):
        light = activity.graph_url(date(2026, 8, 2), dark=False)
        dark = activity.graph_url(date(2026, 8, 2), dark=True)

        self.assertIn("line=599981", light)
        self.assertIn("point=599981", light)
        self.assertIn("area_color=599981", light)
        self.assertIn("bg_color=0d1117", dark)
        self.assertIn("line=599981", dark)
